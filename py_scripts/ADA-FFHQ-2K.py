...
TRAIN_RUNS_DIR_2K = f'{Path(TRAIN_RUNS_DIR)}_2K'
DATA_CUSTOM_DIR_2K = f'{Path(DATA_CUSTOM_DIR)}_2K'

raw_imgs = glob(f'{RAW_IMGS_DIR}/*')
print(len(raw_imgs))
print(f'Raw: {len(raw_imgs)}, Resized: {len(glob(f"{RESIZED_IMGS_DIR}/*"))}')

DeepFAMS.preprocessing.tf_record_exporter(tfrecord_dir=DATA_CUSTOM_DIR_2K,
...
# --outdir={TRAIN_RUNS_DIR_2K} \
# --gpus=2 \
# --data={DATA_CUSTOM_DIR_2K} \
# --snap=1 \
# --kimg=1''', PROJ_DIR)

for num in range(-1, -10, -1):
    files = DeepFAMS.utils.last_snap(num, TRAIN_RUNS_DIR_2K)
    if files != []:
        break
...
run_desc, training_options = DeepFAMS.setup_training_options(
    gpus=2, snap=30, data=DATA_CUSTOM_DIR_2K, resume=latest_snap)

DeepFAMS.RunTraining(outdir=TRAIN_RUNS_DIR_2K,
                     seed=1000,
                     dry_run=True,
                     run_desc=run_desc,
