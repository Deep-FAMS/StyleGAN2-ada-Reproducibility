...
DATA_CUSTOM_DIR_u = f'{PROJ_DIR}/datasets/cifar10u'
TRAIN_RUNS_DIR_u = f'{Path(TRAIN_RUNS_DIR).parent}/unconditional_{Path(TRAIN_RUNS_DIR).name}'
...
for num in range(-1, -10, -1):
    files = DeepFAMS.utils.last_snap(num, TRAIN_RUNS_DIR_u)
    if files != []:
        break
...
DeepFAMS.RunTraining(outdir=TRAIN_RUNS_DIR_u, seed=1000,
