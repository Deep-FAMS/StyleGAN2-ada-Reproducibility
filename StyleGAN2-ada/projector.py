...
class Projector:
    ...
    def set_network(self, Gs, dtype='float16'):
        if Gs is None:
            self._Gs = None
            return
        self._Gs = Gs.clone(randomize_noise=False, dtype=dtype,
                            num_fp16_res=0, fused_modconv=True)
        ...
        self._dlatent_avg = np.mean(
            dlatent_samples, axis=0, keepdims=True)      # [1, 1, C]
        self._dlatent_std = (np.sum(
            (dlatent_samples - self._dlatent_avg) ** 2) / self.dlatent_avg_samples) ** 0.5
        ...
        self._dlatents_var = tf.Variable(tf.zeros(
            [self._minibatch_size] + list(self._dlatent_avg.shape[1:])), name='dlatents_var')
        self._dlatent_noise_in = tf.placeholder(
            tf.float32, [], name='noise_in')
        ...
        self._images_float_expr = tf.cast(
            self._Gs.components.synthesis.get_output_for(self._dlatents_expr), tf.float32)
        ...
        if sh[2] > 256:
            ...
            proc_images_expr = tf.reduce_mean(tf.reshape(
                proc_images_expr, [-1, sh[1], sh[2] // factor, factor, sh[2] // factor, factor]), axis=[3, 5])
    ...
    def start(self, target_images):
        ...
        if sh[2] > self._target_images_var.shape[2]:
            ...
            target_images = np.reshape(
                target_images, [-1, sh[1], sh[2] // factor, factor, sh[3] // factor, factor]).mean((3, 5))
    ...
    def step(self):
        ...
        dlatent_noise = self._dlatent_std * self.initial_noise_factor * \
        ...
        feed_dict = {self._dlatent_noise_in: dlatent_noise,
...
def project(network_pkl: str, target_fname: str, outdir: str, save_video: bool, seed: int):
    ...
    target_float = target_uint8.astype(
        np.float32).transpose([2, 0, 1]) * (2 / 255) - 1
        ...
        for step in t:
            ...
            if writer is not None:
                writer.append_data(np.concatenate(
                    [target_uint8, proj.images_uint8[0]], axis=1))
            dist, loss = proj.step()
...
def main():
    ...
    parser.add_argument('--target',      help='Target image file to project to',
                        dest='target_fname', required=True)
    parser.add_argument(
        '--save-video',  help='Save an mp4 video of optimization progress (default: true)', type=_str_to_bool, default=True)
    parser.add_argument('--seed',        help='Random seed',
                        type=int, default=303)
    parser.add_argument(
