...
def get_weight(shape, gain=1, equalized_lr=True, lrmul=1, weight_var='weight', trainable=True, use_spectral_norm=False):
    ...
    w = tf.get_variable(weight_var, shape=shape,
                        initializer=init, trainable=trainable) * runtime_coef
...
def apply_bias_act(x, act='linear', gain=None, lrmul=1, clamp=None, bias_var='bias', trainable=True):
    b = tf.get_variable(bias_var, shape=[
                        x.shape[1]], initializer=tf.initializers.zeros(), trainable=trainable) * lrmul
...
def dense_layer(x, fmaps, lrmul=1, weight_var='weight', trainable=True, use_spectral_norm=False):
    ...
    w = get_weight([x.shape[1].value, fmaps], lrmul=lrmul, weight_var=weight_var,
                   trainable=trainable, use_spectral_norm=use_spectral_norm)
    w = tf.cast(w, x.dtype)
...
def conv2d(x, w, up=False, down=False, resample_kernel=None, padding=0):
    ...
    w = tf.cast(w, x.dtype)
    if up:
        x = upsample_conv_2d(x, w, data_format='NCHW',
                             k=resample_kernel, padding=padding)
    elif down:
        x = conv_downsample_2d(x, w, data_format='NCHW',
                               k=resample_kernel, padding=padding)
    else:
        padding_mode = {0: 'SAME', -(kernel // 2): 'VALID'}[padding]
        x = tf.nn.conv2d(x, w, data_format='NCHW', strides=[
...
def conv2d_layer(x, fmaps, kernel, up=False, down=False, resample_kernel=None, lrmul=1, trainable=True, use_spectral_norm=False):
    w = get_weight([kernel, kernel, x.shape[1].value, fmaps], lrmul=lrmul,
                   trainable=trainable, use_spectral_norm=use_spectral_norm)
...
def modulated_conv2d_layer(x, y, fmaps, kernel, up=False, down=False, demodulate=True, resample_kernel=None, lrmul=1, fused_modconv=False, trainable=True, use_spectral_norm=False):
    ...
    w = get_weight(wshape, lrmul=lrmul, trainable=trainable,
                   use_spectral_norm=use_spectral_norm)
    if x.dtype.name == 'float16' and not fused_modconv and demodulate:
        # Pre-normalize to avoid float16 overflow.
        w *= np.sqrt(1 / np.prod(wshape[:-1])) / \
            tf.reduce_max(tf.abs(w), axis=[0, 1, 2])
    ww = w[np.newaxis]  # [BkkIO] Introduce minibatch dimension.

    # Modulate.
    # [BI] Transform incoming W to style.
    s = dense_layer(y, fmaps=x.shape[1].value, weight_var='mod_weight',
                    trainable=trainable, use_spectral_norm=use_spectral_norm)
    # [BI] Add bias (initially 1).
    s = apply_bias_act(s, bias_var='mod_bias', trainable=trainable) + 1
    if x.dtype.name == 'float16' and not fused_modconv and demodulate:
        # Pre-normalize to avoid float16 overflow.
        s *= 1 / tf.reduce_max(tf.abs(s))
    ...
    # 2D convolution.
    x = conv2d(x, tf.cast(w, x.dtype), up=up, down=down,
...
def apply_spectral_norm(w, state_var='sn', iterations=1, eps=1e-8):
    ...
    u_var = tf.get_variable(state_var, shape=[
                            1, fmaps], initializer=tf.initializers.random_normal(), trainable=False)
...
def G_main(
    ...
    latents_in,
    # Second input: Conditioning labels [minibatch, label_size].
    labels_in,

    # Evaluation mode.
    # Network is under training? Enables and disables specific features.
    is_training=False,
    # Network is under validation? Chooses which value to use for truncation_psi.
    is_validation=False,
    # Return dlatents (W) in addition to the images?
    ...
    # Value for truncation_psi to use during validation.
    truncation_psi_val=None,
    # Value for truncation_cutoff to use during validation.
    ...
    # Build func name for the mapping network.
    mapping_func='G_mapping',
    # Build func name for the synthesis network.
    synthesis_func='G_synthesis',
    ...
    **kwargs,
    ...
    if 'mapping' not in components:
        components.mapping = tflib.Network('G_mapping', func_name=globals()[
                                           mapping_func], dlatent_broadcast=num_layers, **kwargs)
    ...
    dlatent_avg = tf.get_variable('dlatent_avg', shape=[
                                  dlatent_size], initializer=tf.initializers.zeros(), trainable=False)
    if dlatent_avg_beta is not None:
        with tf.variable_scope('DlatentAvg'):
            batch_avg = tf.reduce_mean(dlatents[:, 0], axis=0)
            update_op = tf.assign(dlatent_avg, tflib.lerp(
    ...
    if style_mixing_prob is not None:
        with tf.variable_scope('StyleMix'):
            ...
            mixing_cutoff = tf.cond(
                ...
                lambda: num_layers)
            dlatents = tf.where(tf.broadcast_to(
                layer_idx < mixing_cutoff, tf.shape(dlatents)), dlatents, dlatents2)
...
def G_mapping(
    ...
    # Number of activations in the mapping layers, None = same as dlatent_size.
    mapping_fmaps=None,
    # Learning rate multiplier for the mapping layers.
    mapping_lrmul=0.01,
    # Activation function: 'relu', 'lrelu', etc.
    mapping_nonlinearity='lrelu',
    # Normalize latent vectors (Z) before feeding them to the mapping layers?
    normalize_latents=True,
    # Label embedding dimensionality, None = same as latent_size.
    label_fmaps=None,
    # Data type to use for intermediate activations and outputs.
    dtype='float32',
    ...
    **_kwargs,                              # Ignore unrecognized keyword args.
    ...
    for layer_idx in range(mapping_layers):
        with tf.variable_scope(f'Dense{layer_idx}'):
            fmaps = mapping_fmaps if mapping_fmaps is not None and layer_idx < mapping_layers - \
                1 else dlatent_size
            x = apply_bias_act(dense_layer(
                x, fmaps=fmaps, lrmul=mapping_lrmul), act=mapping_nonlinearity, lrmul=mapping_lrmul)
...
def G_synthesis(
    ...
    # Number of feature maps in the constant input layer. None = default.
    fmap_const=None,
    ...
    num_fp16_res=0,
    # Clamp the output of convolution layers to [-conv_clamp, +conv_clamp], None = disable clamping.
    conv_clamp=None,
    # Low-pass filter to apply when resampling activations, None = box filter.
    resample_kernel=[1, 3, 3, 1],
    # Implement modulated_conv2d_layer() using grouped convolution?
    fused_modconv=False,
    ...
    **_kwargs,                          # Ignore unrecognized keyword args.
    ...
    if use_noise:
        for layer_idx in range(num_layers - 1):
            ...
            noise_inputs.append(tf.get_variable(
                f'noise{layer_idx}', shape=shape, initializer=tf.initializers.random_normal(), trainable=False))

    # Single convolution layer with all the bells and whistles.
    def layer(x, layer_idx, fmaps, kernel, up=False):
        x = modulated_conv2d_layer(x, dlatents_in[:, layer_idx], fmaps=fmaps, kernel=kernel,
                                   up=up, resample_kernel=resample_kernel, fused_modconv=fused_modconv)
        if use_noise:
            if randomize_noise:
    ...
    def block(x, res):  # res = 3..resolution_log2
        x = tf.cast(x, 'float16' if res > resolution_log2 -
                    num_fp16_res else dtype)
        t = x
        ...
        if architecture == 'resnet':
            with tf.variable_scope('Skip'):
                t = conv2d_layer(t, fmaps=nf(res-1), kernel=1,
                                 up=True, resample_kernel=resample_kernel)
                x = (x + t) * (1 / np.sqrt(2))
        return x
    ...
    def torgb(x, y, res):  # res = 2..resolution_log2
        with tf.variable_scope('ToRGB'):
            t = modulated_conv2d_layer(
                x, dlatents_in[:, res*2-3], fmaps=num_channels, kernel=1, demodulate=False, fused_modconv=fused_modconv)
    ...
    with tf.variable_scope('4x4'):
        with tf.variable_scope('Const'):
            ...
            x = tf.get_variable(
                'const', shape=[1, fmaps, 4, 4], initializer=tf.initializers.random_normal())
...
def D_main(
    # First input: Images [minibatch, channel, height, width].
    images_in,
    # Second input: Conditioning labels [minibatch, label_size].
    labels_in,

    # Input dimensions.
    # Number of input color channels. Overridden based on dataset.
    num_channels=3,
    resolution=1024,         # Input resolution. Overridden based on dataset.
    # Dimensionality of the labels, 0 if no labels. Overridden based on dataset.
    label_size=0,
    ...
    # Internal details.
    # Number of additional mapping layers for the conditioning labels.
    mapping_layers=0,
    # Number of activations in the mapping layers, None = default.
    mapping_fmaps=None,
    # Learning rate multiplier for the mapping layers.
    mapping_lrmul=0.1,
    ...
    # Comparison methods.
    # AdaptiveAugment.get_strength_var() for pagan & adropout.
    augment_strength=0,
    use_pagan=False,        # pagan: Enable?
    # pagan: Number of active bits with augment_strength=1.
    pagan_num=16,
    pagan_fade=0.5,          # pagan: Relative duration of fading in new bits.
    # auxrot: Number of scalars to output. Can vary between evaluations.
    score_size=1,
    # auxrot: Maximum number of scalars to output. Must be set at construction time.
    score_max=1,
    use_spectral_norm=False,        # spectralnorm: Enable?
    # adropout: Standard deviation to use with augment_strength=1, 0 = disable.
    adaptive_dropout=0,
    ...
    **_kwargs,                          # Ignore unrecognized keyword args.
    ...
    if label_size > 0:
        ...
        for idx in range(mapping_layers):
            with tf.variable_scope(f'Mapping{idx}'):
                y = apply_bias_act(dense_layer(
                    y, fmaps=mapping_fmaps, lrmul=mapping_lrmul), act=act, lrmul=mapping_lrmul)
        labels_in = y
    ...
    def adrop(x):
        if adaptive_dropout != 0:
            ...
            x *= tf.cast(tf.exp(tf.random_normal(s) *
    ...
    if use_pagan:
        with tf.variable_scope('PAGAN'):
            ...
            active = (augment_strength * pagan_num - idx - 1) / \
                max(pagan_fade, 1e-8) + 1
            ...
            pagan_signs = tf.reduce_prod(
                1 - pagan_bits * 2, axis=1, keepdims=True)

    # FromRGB block.
    def fromrgb(x, y, res):  # res = 2..resolution_log2
        with tf.variable_scope('FromRGB'):
            trainable = is_next_layer_trainable()
            t = tf.cast(y, 'float16' if res > resolution_log2 -
                        num_fp16_res else dtype)
            t = adrop(conv2d_layer(t, fmaps=nf(res-1),
                      kernel=1, trainable=trainable))
            if pagan_bits is not None:
                with tf.variable_scope('PAGAN'):
                    t += dense_layer(tf.cast(pagan_bits, t.dtype), fmaps=nf(res-1),
                                     trainable=trainable)[:, :, np.newaxis, np.newaxis]
            t = apply_bias_act(t, act=act, clamp=conv_clamp,
                               trainable=trainable)
            if x is not None:
                t += tf.cast(x, t.dtype)
            return t
    ...
    def block(x, res):  # res = 2..resolution_log2
        ...
        with tf.variable_scope('Conv0'):
            trainable = is_next_layer_trainable()
            x = apply_bias_act(adrop(conv2d_layer(x, fmaps=nf(res-1), kernel=3, trainable=trainable,
                               use_spectral_norm=use_spectral_norm)), act=act, clamp=conv_clamp, trainable=trainable)
        with tf.variable_scope('Conv1_down'):
            trainable = is_next_layer_trainable()
            x = apply_bias_act(adrop(conv2d_layer(x, fmaps=nf(res-2), kernel=3, down=True, resample_kernel=resample_kernel,
                               trainable=trainable, use_spectral_norm=use_spectral_norm)), act=act, clamp=conv_clamp, trainable=trainable)
        if architecture == 'resnet':
            with tf.variable_scope('Skip'):
                trainable = is_next_layer_trainable()
                t = adrop(conv2d_layer(t, fmaps=nf(res-2), kernel=1, down=True,
                          resample_kernel=resample_kernel, trainable=trainable))
                x = (x + t) * (1 / np.sqrt(2))
        return x
        ...
        if mbstd_num_features > 0:
            with tf.variable_scope('MinibatchStddev'):
                x = minibatch_stddev_layer(
                    x, mbstd_group_size, mbstd_num_features)
        with tf.variable_scope('Conv'):
            trainable = is_next_layer_trainable()
            x = apply_bias_act(adrop(conv2d_layer(x, fmaps=nf(1), kernel=3, trainable=trainable,
                               use_spectral_norm=use_spectral_norm)), act=act, clamp=conv_clamp, trainable=trainable)
        with tf.variable_scope('Dense0'):
            trainable = is_next_layer_trainable()
            x = apply_bias_act(adrop(dense_layer(x, fmaps=nf(
                0), trainable=trainable)), act=act, trainable=trainable)
