...
class TFRecordDataset:
    def __init__(self,
                 ...
                 max_images=None,
                 # Maximum size of the validation set, None = use all available images.
        ...
        all_files = [f for f in all_files if os.path.basename(
            f).startswith('validation-') == _is_validation]
    ...
    def get_minibatch_tf(self):
        ...
        if self.mirror_augment:
            ...
            images = tf.where(tf.random_uniform(
                [tf.shape(images)[0]]) < 0.5, images, tf.reverse(images, [3]))
    ...
    def load_validation_set_np(self):
        ...
        if self.has_validation_set:
            validation_set = TFRecordDataset(
                tfrecord_dir=self.tfrecord_dir, resolution=self.shape[
                    2], max_label_size=self.label_size,
            ...
            while True:
                ...
                images.append(image)
                labels.append(label)
        images = np.concatenate(images, axis=0) if len(
            images) else np.zeros([0] + self.shape, dtype=self.dtype)
        labels = np.concatenate(labels, axis=0) if len(
            labels) else np.zeros([0, self.label_size], self.label_dtype)
