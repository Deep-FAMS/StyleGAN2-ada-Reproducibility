...
class AdaptiveAugment:
    def __init__(self,
                 ...
                 apply_func=None,
                 # Keyword arguments for the augmentation pipeline.
                 apply_args={},
                 # Augmentation strength (p) to use initially.
                 initial_strength=0,
                 # Heuristic for tuning the augmentation strength dynamically: 'rt', 'rv', None.
                 tune_heuristic=None,
                 # Target value for the selected heuristic.
                 tune_target=None,
        ...
        assert apply_func is None or isinstance(
            apply_func, str) or dnnlib.util.is_top_level_function(apply_func)

        # Configuration.
        self.apply_func = dnnlib.util.get_obj_by_name(
            apply_func) if isinstance(apply_func, str) else apply_func
        ...
        with tflib.absolute_name_scope('Validation'), tf.control_dependencies(None):
            with tf.device('/cpu:0'):
                self._valid_images_in = tf.placeholder(
                    training_set.dtype, name='valid_images_in', shape=[None]+training_set.shape)
                self._valid_labels_in = tf.placeholder(
                    training_set.label_dtype, name='valid_labels_in', shape=[None, training_set.label_size])
            ...
            for gpu, (D_gpu, images_in_gpu, labels_in_gpu) in enumerate(zip(D_gpus, images_in_gpus, labels_in_gpus)):
                with tf.device(f'/gpu:{gpu}'):
                    images_expr = tf.cast(
                        images_in_gpu, tf.float32) * (2 / 255) - 1
                    D_valid = loss.eval_D(
    ...
    def get_strength_var(self):
        if self._strength_var is None:
            with tflib.absolute_name_scope('Augment'), tf.control_dependencies(None):
                self._strength_var = tf.Variable(np.float32(
                    self.strength), name='strength', trainable=False)
    ...
    def tune(self, nimg_delta):
        acc = {name: self._read_and_decay_acc(
            name, nimg_delta) for name in self._tune_stats}
        ...
        if self.tune_heuristic == 'rv':
            ...
            rv = (acc['Loss/scores/real'] - acc['Loss/scores/valid']) / \
                max(acc['Loss/scores/real'] - acc['Loss/scores/fake'], 1e-8)
    ...
    def run_validation(self, minibatch_size):
        if self._valid_images is not None:
            indices = [(self._valid_ofs + i) % self._valid_images.shape[0]
                       for i in range(minibatch_size)]
            tflib.run(self._valid_op, {
                      self._valid_images_in: self._valid_images[indices], self._valid_labels_in: self._valid_labels[indices]})
    ...
    def _increment_acc(self, name, expr):
        with tf.name_scope('acc_' + name):
            with tf.control_dependencies(None):
                # [acc_num, acc_sum]
                acc_var = tf.Variable(tf.zeros(2), name=name, trainable=False)
        ...
        if nimg_delta > 0:
            with tflib.absolute_name_scope('Augment'), tf.control_dependencies(None):
                ...
                if name not in self._acc_decay_ops:
                    with tf.name_scope('acc_' + name):
                        ops = [tf.assign(var, var * self._acc_decay_in)
                               for var in acc_vars]
                        self._acc_decay_ops[name] = tf.group(*ops)
            acc_decay = 0.5 ** (nimg_delta / (self.stat_decay_kimg * 1000)
                                ) if self.stat_decay_kimg > 0 else 0
            tflib.run(self._acc_decay_ops[name], {
                      self._acc_decay_in: acc_decay})
...
def gate_augment_params(probability, params, disabled_val):
    ...
    disabled_val = tf.broadcast_to(tf.convert_to_tensor(
        disabled_val, dtype=params.dtype), shape)
...
def construct_batch_of_matrices(*rows):
    rows = [[tf.convert_to_tensor(x, dtype=tf.float32)
...
def augment_pipeline(
    # Input images: NCHW, float32, dynamic range [-1,+1].
    images,
    labels,                         # Input labels.
    # Overall multiplier for augmentation probability; can be a Tensor.
    strength=1,
    # Percentile value for visualizing parameter ranges; None = normal operation.
    ...
    if xint > 0:
        ...
        if debug_percentile is not None:
            t = (tf.broadcast_to(debug_percentile,
                 [batch, 2]) * 2 - 1) * xint_max
        G_inv @= translate_2d_inv(tf.rint(t[:, 0]
                                  * width), tf.rint(t[:, 1] * height))
    ...
    if scale > 0:
        ...
        if debug_percentile is not None:
            s = 2 ** (tflib.erfinv(tf.broadcast_to(debug_percentile,
                      [batch]) * 2 - 1) * scale_std)
    ...
    # P(pre OR post) = p
    p_rot = 1 - \
        tf.sqrt(tf.cast(tf.maximum(1 - rotate * strength, 0), tf.float32))
    if rotate > 0:
        theta = tf.random_uniform(
            [batch], -np.pi * rotate_max, np.pi * rotate_max)
        theta = gate_augment_params(p_rot, theta, 0)
        if debug_percentile is not None:
            theta = (tf.broadcast_to(debug_percentile, [
                     batch]) * 2 - 1) * np.pi * rotate_max
    ...
    if xfrac > 0:
        ...
        if debug_percentile is not None:
            t = tflib.erfinv(tf.broadcast_to(
                debug_percentile, [batch, 2]) * 2 - 1) * xfrac_std
    ...
    if G_inv is not I_3:
        ...
        Hz = np.reshape(Hz, [-1, 1, 1]).repeat(channels,
                                               axis=1)  # [tap, channel, 1]
        Hz_pad = Hz.shape[0] // 4

        # Calculate padding.
        cx = (width - 1) / 2
        cy = (height - 1) / 2
        cp = np.transpose([[-cx, -cy, 1], [cx, -cy, 1],
                          [cx, cy, 1], [-cx, cy, 1]])  # [xyz, idx]
        ...
        # Upsample.
        shape = [batch, tf.shape(images)[1] * 2,
                 tf.shape(images)[2] * 2, channels]
        images = tf.nn.depthwise_conv2d_backprop_input(input_sizes=shape, filter=Hz[np.newaxis, :], out_backprop=images, strides=[
                                                       1, 2, 2, 1], padding='SAME', data_format='NHWC')
        images = tf.nn.depthwise_conv2d_backprop_input(input_sizes=shape, filter=Hz[:, np.newaxis], out_backprop=images, strides=[
                                                       1, 1, 1, 1], padding='SAME', data_format='NHWC')
        # Account for the increased resolution.
        G_inv = scale_2d(2, 2) @ G_inv @ scale_2d_inv(2, 2)

        # Execute transformation.
        transforms = tf.reshape(G_inv, [-1, 9])[:, :8]
        shape = [(height + Hz_pad * 2) * 2, (width + Hz_pad * 2) * 2]
        images = tf.contrib.image.transform(
            images=images, transforms=transforms, output_shape=shape, interpolation='BILINEAR')

        # Downsample and crop.
        images = tf.nn.depthwise_conv2d(input=images, filter=Hz[np.newaxis, :], strides=[
                                        1, 1, 1, 1], padding='SAME', data_format='NHWC')
        images = tf.nn.depthwise_conv2d(input=images, filter=Hz[:, np.newaxis], strides=[
                                        1, 2, 2, 1], padding='SAME', data_format='NHWC')
    ...
    if brightness > 0:
        ...
        if debug_percentile is not None:
            b = tflib.erfinv(tf.broadcast_to(debug_percentile,
                             [batch]) * 2 - 1) * brightness_std
    ...
    if contrast > 0:
        ...
        if debug_percentile is not None:
            c = 2 ** (tflib.erfinv(tf.broadcast_to(debug_percentile,
                      [batch]) * 2 - 1) * contrast_std)
        C = scale_3d(c, c, c) @ C
    ...
    if C is not I_4:
        ...
        if channels == 3:
            ...
            images = images * \
                tf.reduce_sum(C[:, :, :3], axis=2, keepdims=True) + C[:, :, 3:]
        else:
    ...
    if imgfilter > 0:
        ...
        expected_power = np.array([10, 1, 1, 1]) / 13
        ...
        for i, band_strength in enumerate(imgfilter_bands):
            ...
            t_i = gate_augment_params(
                imgfilter * strength * band_strength, t_i, 1)
            if debug_percentile is not None:
            ...
            t = tf.concat([t[:, :i], t_i[:, np.newaxis], t[:, i+1:]], axis=-1)
            # Normalize power.
            t /= tf.sqrt(tf.reduce_sum(expected_power *
                         tf.square(t), axis=-1, keepdims=True))
        ...
        for i in range(1, num_bands):
            Hz_bands = np.dstack([Hz_bands, np.zeros_like(
                Hz_bands)]).reshape(num_bands, -1)[:, :-1]
            Hz_bands = scipy.signal.convolve(Hz_bands, [Hz_lo2])
            Hz_bands[i, (Hz_bands.shape[1] - Hz_hi2.size) //
                     2: (Hz_bands.shape[1] + Hz_hi2.size) // 2] += Hz_hi2

        # Construct combined amplification filter.
        Hz_prime = g @ Hz_bands  # [batch, tap]
        Hz_prime = tf.transpose(Hz_prime)  # [tap, batch]
        Hz_prime = tf.tile(Hz_prime[:, :, np.newaxis], [
                           1, 1, channels])  # [tap, batch, channels]
        # [tap, batch * channels, 1]
        Hz_prime = tf.reshape(Hz_prime, [-1, batch * channels, 1])
        ...
        images = tf.pad(tensor=images, paddings=pad, mode='REFLECT')
        images = tf.nn.depthwise_conv2d(input=images, filter=Hz_prime[np.newaxis, :], strides=[
    ...
    if noise > 0:
        ...
        if debug_percentile is not None:
            sigma = tflib.erfinv(tf.broadcast_to(
                debug_percentile, [batch])) * noise_std
    ...
    if cutout > 0:
        ...
        coord_x = tf.reshape(
            tf.range(width, dtype=tf.float32), [1, 1, 1, width])
        coord_y = tf.reshape(
            tf.range(height, dtype=tf.float32), [1, 1, height, 1])
        mask_x = (tf.abs((coord_x + 0.5) / width -
                  center[:, 0]) >= size[:, 0] / 2)
        mask_y = (tf.abs((coord_y + 0.5) / height -
                  center[:, 1]) >= size[:, 1] / 2)
