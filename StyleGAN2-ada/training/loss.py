...
def stylegan2(G, D, aug, fake_labels, real_images, real_labels, r1_gamma=10, pl_minibatch_shrink=2, pl_decay=0.01, pl_weight=2, **_kwargs):
    ...
    if r1_gamma != 0:
        with tf.name_scope('Loss_R1'):
            r1_grads = tf.gradients(tf.reduce_sum(
                D_real.scores), [real_images])[0]
    ...
    if pl_weight != 0:
        with tf.name_scope('Loss_PL'):
            ...
            # Compute |J*y|.
            pl_noise = tf.random_normal(
                tf.shape(G_pl.images)) / np.sqrt(np.prod(G.output_shape[2:]))
            pl_grads = tf.gradients(tf.reduce_sum(
                G_pl.images * pl_noise), [G_pl.dlatents])[0]
            pl_lengths = tf.sqrt(tf.reduce_mean(
                tf.reduce_sum(tf.square(pl_grads), axis=2), axis=1))
...
def cmethods(G, D, aug, fake_labels, real_images, real_labels,
    ...
    if r1_gamma != 0 or r2_gamma != 0:
        with tf.name_scope('Loss_R1R2'):
            if r1_gamma != 0:
                ...
                D_reg += r1_penalty * (r1_gamma * 0.5)
            if r2_gamma != 0:
                r2_grads = tf.gradients(tf.reduce_sum(
                    D_fake.scores), [G_fake.images])[0]
    ...
    if (bcr_real_weight != 0 or bcr_fake_weight != 0) and bcr_augment is not None:
        with tf.name_scope('Loss_bCR'):
            if bcr_real_weight != 0:
                ...
                D_bcr_real = eval_D(
                    D, aug, bcr_real_images, bcr_real_labels, report='real_bcr', augment_inputs=False)
            ...
            if bcr_fake_weight != 0:
                ...
                D_bcr_fake = eval_D(
                    D, aug, bcr_fake_images, bcr_fake_labels, report='fake_bcr', augment_inputs=False)
    ...
    if zcr_gen_weight != 0 or zcr_dis_weight != 0:
        with tf.name_scope('Loss_zCR'):
            zcr_fake_latents = fake_latents + \
                tf.random_normal([minibatch_size] +
                                 G.input_shapes[0][1:]) * zcr_noise_std
            G_zcr = eval_G(G, zcr_fake_latents, fake_labels)
            if zcr_gen_weight > 0:
                zcr_gen_penalty = - \
                    tf.reduce_mean(tf.square(G_fake.images -
                                   G_zcr.images), axis=[1, 2, 3])
                zcr_gen_penalty = report_stat(
                    aug, 'Loss/zcr_gen_penalty', zcr_gen_penalty)
                G_loss += zcr_gen_penalty * zcr_gen_weight
            if zcr_dis_weight > 0:
                D_zcr = eval_D(D, aug, G_zcr.images, fake_labels,
                               report='fake_zcr', augment_inputs=False)
    ...
    if auxrot_alpha != 0 or auxrot_beta != 0:
        with tf.name_scope('Loss_AuxRot'):
            idx = tf.range(minibatch_size * 4,
                           dtype=tf.int32) // minibatch_size
            ...
            if auxrot_alpha != 0:
                ...
                auxrot_fake = tf.where(
                    b0, auxrot_fake, tf.reverse(auxrot_fake, [2]))
                auxrot_fake = tf.where(
                    b1, auxrot_fake, tf.reverse(auxrot_fake, [3]))
                auxrot_fake = tf.where(
                    b2, auxrot_fake, tf.transpose(auxrot_fake, [0, 1, 3, 2]))
                D_auxrot_fake = eval_D(
                    D, aug, auxrot_fake, fake_labels, return_aux=4)
                G_loss += tf.nn.sparse_softmax_cross_entropy_with_logits(
                    labels=idx, logits=D_auxrot_fake.aux) * auxrot_alpha
            if auxrot_beta != 0:
                auxrot_real = tf.tile(real_images, [4, 1, 1, 1])
                auxrot_real = tf.where(
                    b0, auxrot_real, tf.reverse(auxrot_real, [2]))
                auxrot_real = tf.where(
                    b1, auxrot_real, tf.reverse(auxrot_real, [3]))
                auxrot_real = tf.where(
                    b2, auxrot_real, tf.transpose(auxrot_real, [0, 1, 3, 2]))
                D_auxrot_real = eval_D(
                    D, aug, auxrot_real, real_labels, return_aux=4)
                D_loss += tf.nn.sparse_softmax_cross_entropy_with_logits(
                    labels=idx, logits=D_auxrot_real.aux) * auxrot_beta
...
def wgangp(G, D, aug, fake_labels, real_images, real_labels, wgan_epsilon=0.001, wgan_lambda=10, wgan_target=1, **_kwargs):
    ...
    with tf.name_scope('Loss_GP'):
        ...
        # NOTE: Mixing is performed without respect to fake_labels.
        ...
        mix_norms = tf.sqrt(tf.reduce_sum(
            tf.square(mix_grads), axis=[1, 2, 3]))
