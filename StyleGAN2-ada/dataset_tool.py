...
class HDF5Exporter:
    def __init__(self, h5_filename, resolution, channels, compress=False, expected_images=0, print_progress=True, progress_interval=10):
        ...
        for lod in range(rlog2, -1, -1):
            ...
            self.buffers.append(
                np.zeros((buffer_size, c, r, r), dtype=np.uint8))
    ...
    def add_images(self, img):
        ...
        for lod in range(len(self.h5_lods)):
            ...
            while ofs < quant.shape[0]:
                num = min(
                    quant.shape[0] - ofs, self.buffers[lod].shape[0] - self.buffer_sizes[lod])
        ...
        with TFRecordExporter(tfrecord_dir, max_images, print_progress=False) as tfr:
            for idx, (_key, value) in enumerate(txn.cursor()):
                try:
                    ...
                    canvas[:, (width - height) //
                           2: (width + height) // 2] = img
                    tfr.add_image(canvas)
                    print('\r%d / %d => %d ' %
                          (idx + 1, total_images, tfr.cur_images), end='')
...
def create_from_hdf5(tfrecord_dir, hdf5_filename, shuffle):
    ...
    with h5py.File(hdf5_filename, 'r') as hdf5_file:
        ...
        with TFRecordExporter(tfrecord_dir, hdf5_data.shape[0]) as tfr:
            order = tfr.choose_shuffled_order(
            ) if shuffle else np.arange(hdf5_data.shape[0])
            for idx in range(order.size):
...
def unpack(tfrecord_dir, output_dir, resolution=None):
    ...
    while True:
        ...
        img.save(png_fname)
        idx += 1
    np.savez(f'{output_dir}/pack_extras.npz',
             labels=np.array(labels, dtype=np.uint8), num_images=idx)
    print('Extracted %d images.' % idx)
...
def pack(unpacked_dir, tfrecord_dir, num_train=None, num_validation=None, mirror=0, seed=None):
        ...
        with TFRecordExporter(tfrecord_dir, len(source_idx), tfr_prefix=tfr_prefix) as tfr:
            for mirror, idx in source_idx:
                img = np.asarray(PIL.Image.open(
                    make_png_path(unpacked_dir, idx)))
                img = img.transpose([2, 0, 1])  # HWC => CHW
                if mirror != 0:
                    img = img[:, :, ::-1]
                tfr.add_image(img)
    ...
    assert (labels_onehot.shape[0] == num_images) and (
        len(labels_onehot.shape) == 2)
...
def extract_brecahad_crops(brecahad_dir, output_dir, cropsize=256):
    ...
    if cropsize not in params:
        print('--cropsize must be one of:', ', '.join(str(x)
...
def execute_cmdline(argv):
    ...
    p.add_argument('tfrecord_dir',
                   help='Dataset directory to load the images from')
    p.add_argument('--compress',
                   help='Compress the data (default: 0)', type=int, default=0)
    ...
    p = add_command(
        'unpack',           'Unpack a TFRecords dataset to labels and images for later repackaging with `pack`.')
    p.add_argument('--tfrecord_dir',
                   help='Directory containing the source dataset in TFRecords format', required=True)
    p.add_argument('--output_dir',
                   help='Output directory where to extract the dataset as PNG files', required=True)
    p.add_argument('--resolution',
                   help='Resize images to (resolution,resolution) (default: None = no resizing)', type=int, default=None)
    ...
    p.add_argument('--tfrecord_dir',
                   help='New dataset directory to be created')
    p.add_argument('--num_train',
                   help='Number of images to pick for the training set (default: None = all)', type=int, default=None)
    p.add_argument('--num_validation',
                   help='Number of images to pick for the validation set (default: 0 = no images)', type=int, default=0)
    p.add_argument('--mirror',
                   help='Number of images to pick for the training set (default: 0 = no mirroring)', type=int, default=0)
    p.add_argument('--seed',
                   help='Shuffle random seed.  (default: None = do not shuffle)', type=int, default=None)

    p = add_command('extract_brecahad_crops',
                    'Extract crops from the original BreCaHAD images')
    p.add_argument('--brecahad_dir',
                   help='Source directory for BreCaHAD images.  Should contain .tif files.', required=True)
    p.add_argument('--output_dir',
                   help='Output directory for image crops.  Will contain .png files', required=True)
    p.add_argument('--cropsize',
                   help='Crop size (resolution,resolution)', type=int, default=256)
