...
def _fused_bias_act_cuda(x, b, axis, act, alpha, gain, clamp):
    ...
    cuda_kwargs = dict(axis=int(axis), act=int(
        act_spec.cuda_idx), gain=float(gain))
    ...
    def func_y(x, b):
        y = cuda_op(x=x, b=b, xref=empty_tensor,
                    yref=empty_tensor, grad=0, **cuda_kwargs)
        y.set_shape(x.shape)
        return y

    # Backward pass: dx, db = grad(dy, x, y)
    def grad_dx(dy, x, y):
        dx = cuda_op(x=dy, b=empty_tensor, xref=ref(x, 'x'),
                     yref=ref(y, 'y'), grad=1, **cuda_kwargs)
    ...
    def grad2_d_dy(d_dx, d_db, x, y):
        d_dy = cuda_op(x=d_dx, b=d_db, xref=ref(x, 'x'),
                       yref=ref(y, 'y'), grad=1, **cuda_kwargs)
        d_dy.set_shape(x.shape)
        return d_dy

    def grad2_d_x(d_dx, d_db, x, y):
        d_x = cuda_op(x=d_dx, b=d_db, xref=ref(x, 'x'),
                      yref=ref(y, 'y'), grad=2, **cuda_kwargs)
