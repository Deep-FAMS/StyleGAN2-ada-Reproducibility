...
def _upfirdn_2d_cuda(x, k, upx, upy, downx, downy, padx0, padx1, pady0, pady1):
    ...
    @tf.custom_gradient
    def func(x):
        y = cuda_op(x=x, k=kc, upx=int(upx), upy=int(upy), downx=int(downx), downy=int(
            downy), padx0=int(padx0), padx1=int(padx1), pady0=int(pady0), pady1=int(pady1))
        y.set_shape([majorDim, outH, outW, minorDim])

        @tf.custom_gradient
        def grad(dy):
            dx = cuda_op(x=dy, k=gkc, upx=int(downx), upy=int(downy), downx=int(upx), downy=int(
                upy), padx0=int(gpadx0), padx1=int(gpadx1), pady0=int(gpady0), pady1=int(gpady1))
...
def upsample_2d(x, k=None, factor=2, gain=1, padding=0, data_format='NCHW', impl='cuda'):
    ...
    assert isinstance(padding, int)
    k = _FilterKernel(k if k is not None else [
                      1] * factor, gain * (factor ** 2))
    assert k.w == k.h
...
def upsample_conv_2d(x, w, k=None, factor=2, gain=1, padding=0, data_format='NCHW', impl='cuda'):
    ...
    if cw == 1 and ch == 1:
        x = tf.nn.conv2d(x, w, data_format=data_format,
                         strides=[1, 1, 1, 1], padding='VALID')
        x = upsample_2d(x, k, factor=factor, gain=gain,
    ...
    if data_format == 'NCHW':
        ...
        output_shape = [_shape(x, 0), outC, (_shape(
            x, 2) - 1) * factor + ch, (_shape(x, 3) - 1) * factor + cw]
        num_groups = _shape(x, 1) // inC
    else:
        stride = [1, factor, factor, 1]
        output_shape = [_shape(x, 0), (_shape(x, 1) - 1) *
                        factor + ch, (_shape(x, 2) - 1) * factor + cw, outC]
...
def conv_downsample_2d(x, w, k=None, factor=2, gain=1, padding=0, data_format='NCHW', impl='cuda'):
    ...
    # Fast path for 1x1 convolution.
    if cw == 1 and ch == 1:
        x = downsample_2d(x, k, factor=factor, gain=gain,
                          padding=padding, data_format=data_format, impl=impl)
        x = tf.nn.conv2d(x, w, data_format=data_format,
                         strides=[1, 1, 1, 1], padding='VALID')
        return x
    ...
    x = _simple_upfirdn_2d(x, k, pad0=pad0, pad1=pad1,
...
def _simple_upfirdn_2d(x, k, up=1, down=1, pad0=0, pad1=0, data_format='NCHW', impl='cuda'):
    ...
    if k.kx is not None:
        y = upfirdn_2d(y, k.kx, upx=up, downx=down,
                       padx0=pad0, padx1=pad1, impl=impl)
    if k.ky is not None:
        y = upfirdn_2d(y, k.ky, upy=up, downy=down,
                       pady0=pad0, pady1=pad1, impl=impl)
    if k.kxy is not None:
        y = upfirdn_2d(y, k.kxy, upx=up, upy=up, downx=down, downy=down,
