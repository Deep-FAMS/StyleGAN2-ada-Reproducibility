...
class MetricBase:
    ...
    def _get_cache_file_for_reals(self, extension='pkl', **kwargs):
        ...
        dataset_name = self._dataset_args.get(
            'tfrecord_dir', None) or self._dataset_args.get('h5_file', None)
    ...
    def _iterate_fakes(self, Gs, minibatch_size, num_gpus):
        while True:
            ...
            images = Gs.run(latents, None, output_transform=fmt,
                            is_validation=True, num_gpus=num_gpus, assume_frozen=True)
            yield images
...
class MetricGroup:
    def __init__(self, metric_kwarg_list):
        self.metrics = [dnnlib.util.call_func_by_name(
            **kwargs) for kwargs in metric_kwarg_list]
