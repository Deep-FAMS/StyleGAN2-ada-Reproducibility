...
def mutual_information(p):
    ...
    for x in range(p.shape[0]):
        ...
        for y in range(p.shape[1]):
            ...
            if p_xy > 0.0:
                # get bits as output
                result += p_xy * np.log2(p_xy / (p_x * p_y))
    return result
...
def conditional_entropy(p):
    ...
    return max(0.0, entropy(y) - mutual_information(p))
...
class LS(metric_base.MetricBase):
    ...
    def _evaluate(self, Gs, Gs_kwargs, num_gpus):
        ...
        results = {key: np.concatenate(
            [value[key] for value in results], axis=0) for key in results[0].keys()}
        ...
        for attrib_idx in self.attrib_indices:
            ...
            svm_targets = np.argmax(
                results[attrib_idx][pruned_indices], axis=1)
            for space in ['latents', 'dlatents']:
                ...
                p = [[np.mean([case == (row, col) for case in zip(
                    svm_outputs, svm_targets)]) for col in (0, 1)] for row in (0, 1)]
                conditional_entropies[space].append(conditional_entropy(p))

        # Calculate separability scores.
        scores = {key: 2**np.sum(values)
                  for key, values in conditional_entropies.items()}
