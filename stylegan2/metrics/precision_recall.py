    ...
    def __init__(self, num_features, num_gpus):
        ...
        with tf.device('/cpu:0'):
            ...
            self._features_batch2 = tf.placeholder(
                tf.float16, shape=[None, self.num_features])
            features_split2 = tf.split(
                self._features_batch2, self.num_gpus, axis=0)
            distances_split = []
            for gpu_idx in range(self.num_gpus):
                with tf.device('/gpu:%d' % gpu_idx):
                    distances_split.append(batch_pairwise_distances(
...
class ManifoldEstimator():
    ...
    def __init__(self, distance_block, features, row_batch_size, col_batch_size, nhood_sizes, clamp_to_percentile=None):
        ...
        distance_batch = np.zeros(
            [row_batch_size, num_images], dtype=np.float16)
        ...
        for begin1 in range(0, num_images, row_batch_size):
            ...
            for begin2 in range(0, num_images, col_batch_size):
                ...
                distance_batch[0:end1-begin1,
                               begin2:end2] = self._distance_block.pairwise_distances(row_batch, col_batch)
    ...
    def evaluate(self, eval_features, return_realism=False, return_neighbors=False):
        ...
        for begin1 in range(0, num_eval_images, self.row_batch_size):
            ...
            realism_score[begin1:end1] = self.D[nearest_indices[begin1:end1],
                                                0] / np.min(distance_batch[0:end1-begin1, :], axis=1)
...
class PR(metric_base.MetricBase):
    ...
    def _evaluate(self, Gs, Gs_kwargs, num_gpus):
        ...
        feature_net = misc.load_pkl(
            'https://nvlabs-fi-cdn.nvidia.com/stylegan/networks/metrics/vgg16.pkl')
        ...
        for begin in range(0, self.num_images, minibatch_size):
            ...
            eval_features[begin:end] = np.concatenate(
