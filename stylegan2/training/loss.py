...
def G_logistic(G, D, opt, training_set, minibatch_size):
    ...
    fake_scores_out = D.get_output_for(
        fake_images_out, labels, is_training=True)
...
def D_logistic_r1(G, D, opt, training_set, minibatch_size, reals, labels, gamma=10.0):
    ...
    with tf.name_scope('GradientPenalty'):
        ...
        gradient_penalty = autosummary(
            'Loss/gradient_penalty', gradient_penalty)
...
def D_logistic_r2(G, D, opt, training_set, minibatch_size, reals, labels, gamma=10.0):
    ...
    with tf.name_scope('GradientPenalty'):
        fake_grads = tf.gradients(tf.reduce_sum(
            fake_scores_out), [fake_images_out])[0]
...
def D_wgan_gp(G, D, opt, training_set, minibatch_size, reals, labels, wgan_lambda=10.0, wgan_epsilon=0.001, wgan_target=1.0):
    ...
    with tf.name_scope('GradientPenalty'):
        ...
        mixed_scores_out = D.get_output_for(
            mixed_images_out, labels, is_training=True)
        mixed_scores_out = autosummary('Loss/scores/mixed', mixed_scores_out)
        mixed_grads = tf.gradients(tf.reduce_sum(
            mixed_scores_out), [mixed_images_out])[0]
        mixed_norms = tf.sqrt(tf.reduce_sum(
            tf.square(mixed_grads), axis=[1, 2, 3]))
...
def G_logistic_ns_pathreg(G, D, opt, training_set, minibatch_size, pl_minibatch_shrink=2, pl_decay=0.01, pl_weight=2.0):
    ...
    with tf.name_scope('PathReg'):
        ...
        if pl_minibatch_shrink > 1:
            ...
            pl_latents = tf.random_normal(
                [pl_minibatch] + G.input_shapes[0][1:])
        ...
        pl_noise = tf.random_normal(
            tf.shape(fake_images_out)) / np.sqrt(np.prod(G.output_shape[2:]))
        pl_grads = tf.gradients(tf.reduce_sum(
            fake_images_out * pl_noise), [fake_dlatents_out])[0]
