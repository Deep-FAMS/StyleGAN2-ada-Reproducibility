            ...
            with tfutil.absolute_name_scope(self.scope + "/Apply%d" % device_idx), tf.device(device.name):
                ...
                if device_idx == len(self._devices) - 1:
                    all_ops.append(autosummary.autosummary(
                        self.id + "/learning_rate", tf.convert_to_tensor(self.learning_rate)))
                    all_ops.append(autosummary.autosummary(
    ...
    def _broadcast_nccl(self):
        ...
        for all_vars in zip(*[device.grad_clean.keys() for device in self._devices.values()]):
            if any(x.shape.num_elements() > 0 for x in all_vars):
                all_grads = [device.grad_clean[var] for device,
                             var in zip(self._devices.values(), all_vars)]
    ...
    def _broadcast_fallback(self):
        ...
        for device in self._devices.values():
            with tf.device(device.name):
                combo = [tf.reshape(x, [x.shape.num_elements()])
                ...
                for var, grad_old in device.grad_clean.items():
                    grad_new = tf.reshape(
                        combo[cur_ofs: cur_ofs + grad_old.shape.num_elements()], grad_old.shape)
