...
class Projector:
    ...
    def set_network(self, Gs, minibatch_size=1):
        ...
        self._dlatent_avg = np.mean(
            dlatent_samples, axis=0, keepdims=True)  # [1, 1, 512]
    ...
    def start(self, target_images):
        ...
        tflib.set_vars({self._target_images_var: target_images, self._dlatents_var: np.tile(
    ...
    def step(self):
        ...
        # Train.
        feed_dict = {self._noise_in: noise_strength,
        ...
        if self._cur_step == self.num_steps or self._cur_step % 10 == 0:
            self._info('%-8d%-12g%-12g' %
                       (self._cur_step, dist_value, loss_value))
        if self._cur_step == self.num_steps:
            self._info('Done.')
